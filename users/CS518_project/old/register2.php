...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $useremail_err = "This email is already exists.";
                } else{
    ...
    if(empty($firstname_err) && empty($lastname_err) && empty($useremail_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO users (firstname, lastname, useremail, password) VALUES (?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_firstname, $param_lastname, $param_useremail, $param_password);
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="email" name = "email" class="form-control <?php echo (!empty($useremail_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $useremail; ?>">
